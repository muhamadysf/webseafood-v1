...
if (isset($_POST['selectedId'])) {
    ...
    if ($mode === 'simpan') {
        ...
        $query = $conn->prepare("UPDATE tb_pesanan SET status_pesanan = ?, keterangan = ? WHERE id_pesanan = ? ");
        $query->bind_param("ssi", $status, $keterangan, $id_pesanan);

        if ($query->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Data pesanan dibatalkan']);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Gagal update: ' . $conn->error]);
        }
        // 
